...
from source.sleepTimer.sleepTimerApp import sleepTimerApp
...
class app(ui):
	...
	def initUi( self ):
		...
		self.__events = eventHandler( self.eventTableWidget, self.hourComboBox, self.minuteSpinBox, 
									  self.addEventButton, self.removeEventButton, self.lineEdit, self.dateLabel, self.__ip, self.__port)
	...
	def connectWidgets( self ):
		...
		self.__sleepTimerApp.quitSignalExit.connect( self.enableWindow )
		self.__sleepTimerApp.quitSignalAccept.connect( self.updateSleep )
		...
		self.sleepTimerAction.triggered.connect( self.editSleep )

		self.submitButton.clicked.connect( self.writeWeather )

		for i in range( len( self.speedAction )):
			self.speedAction[i].triggered.connect( lambda _, action=self.speedAction[i]: self.writeSpeed( action ))
		for i in range( len( self.viewAction )):
			self.viewAction[i].triggered.connect( lambda _, action=self.viewAction[i]: self.writeView( action ))
	...
	def editSleep( self ):
		self.setEnabled( False )
		self.__sleepTimerApp.readFromFile( )
		self.__sleepTimerApp.show( super( ).x( ), super( ).y( ))

	def updateSleep( self ):
		self.enableWindow( )
		self.sendToServer( self.__sleepFile )

	def writeSpeed( self, speed ):
		self.__fileIO.simpleWrite( self.__speedFile, speed.text( )[0:-1])
		self.sendToServer( self.__speedFile )
		
	def writeView( self, view ):
		self.__fileIO.simpleWrite( self.__viewFile, view.text( ))
		self.sendToServer( self.__viewFile )
