...
class fileIO:
	def simpleRead( self, fileToOpen, force=False, multiLine=False, separator="" ):
		try:
			...
			print( datetime.now( ).strftime( "%H:%M:%S"), "IN fileIO::simpleRead: could not open file <", fileToOpen, ">" )
			if( force ):
	...
	def conditionalQtRead( self, fileToOpen, listOfQtItems, force=False, returnText=False ):
		...
		try:
			...
			print( datetime.now( ).strftime( "%H:%M:%S"), "IN fileIO::conditionalQtRead: could not open file <", fileToOpen, ">" )
			if( force ):
				self.forceCreate( fileToOpen )
			else:
				return -1
	...
	def conditionalWrite( self, fileToOpen, dictionary, clearContents=True, separator='',newLine=False ):
		...
		nl=''
		if( len( dictionary ) == 0 ):
			print( datetime.now( ).strftime( "%H:%M:%S"), "IN fileIO::conditionalWrite: dictionary must have at least one element.")
			return -1
		file = open( fileToOpen, "a+", encoding="utf-8" )
	...
	def separateTwoStrings( self, text1, text2, separator ):
		if( len( text1 ) != len( text2 )):
			print( datetime.now( ).strftime( "%H:%M:%S"), "IN fileIO::separateTwoStrings: text1 and text2 must be the same length" )
			return -1
		text = []
	...
	def forceCreate( self, fileToOpen ):
		print( datetime.now( ).strftime( "%H:%M:%S"), "Forcing to make <", fileToOpen, ">" )
	...
	def formListOfDictionary( self, listOfBools, listOfData ):
		...
		if( len( listOfBools ) == len( listOfData )):
			...
			print( datetime.now( ).strftime( "%H:%M:%S"), "IN fileIO::formListOfDictionary: listOfBools and listOfData must have the same length" )
			return -1
