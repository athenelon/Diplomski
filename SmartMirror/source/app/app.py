...
from source.app.modules.gpioInterface import gpioInterface#Lines 13,53,202,204,238
...
class app:
	...
	__eventsFile = "/home/pi/SmartMirror/data/events/"
...
#-----------------------------calendar-------news--------events-------weather----------
	...
	def renderDate( self ):
		...
		if( day[0] == '0' ):
			day = day[1:3]
		self.__eventsFile = "/home/pi/SmartMirror/data/events/" + year[:-1] + "/" + mnt + "/" + day + "txt"

		self.__calendar.renderText( self.__color, self.__fonts[0], 0 )
		self.__calendar.renderEventCount( self.__colorInv, self.__fonts[7], "/home/pi/SmartMirror/data/events/" + year[:-1] + "/" + mnt + "/" )
	...
	def renderNews( self ):
		if( self.__socketServer.getNewsFlag( )):
			...
			self.__news.renderText( self.__color, self.__fonts[4], self.__config.getDefFont( ), self.__news.getText( ), self.__step * self.__newsWidth )#self.__step*10.3 )

	def renderWeather( self ):
		if( self.__socketServer.getWeatherFlag( )):
			self.__socketServer.setWeatherFlag( False )
			self.__weather.renderText( self.__color, self.__colorInv, self.__fonts[6], self.__fonts[5], self.__config.getMetFont( ))
	...
	def renderEvents( self ):
		if( self.__socketServer.getEventFlag( )):
			...
			self.__events.renderText( self.__colorInv, self.__fonts[3],self.__config.getDefFont( ) , text, self.__step*4.8 )
	...
	def setConfig( self ):
		if( self.__socketServer.getConfigFlag( ) or self.__gpio.getViewFlag( )):
			self.__socketServer.setConfigFlag( False )
			self.__gpio.setViewFlag( False )
	...
	def runMirror( self ):
		...
		while( self.__run ):
			self.__socketServer.bindServer( )#Only once
			...
			if( self.__gpio.pirSensor( ) and not self.__config.getStayOffFlag( )):
				self.__gpio.setDisplayFlag( False )
				self.__config.displayOn( )
				self.__config.restartDim( )
